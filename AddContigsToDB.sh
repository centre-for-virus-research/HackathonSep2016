...
#
# Usage: AddContigsToDB.sh merged_contigs.txt seqID
# 
...
Usage () {
    echo "";
    echo "Usage: AddContigsToDB.sh merged_contigs.txt seqID";
    echo "";
}
...
echo "DROP TABLE IF EXISTS KnownDark_t; create table KnownDark_t(queryID varchar(100) not null Primary key, subjectID varchar(100), percIdentity float, alnLength int, mismatchCount int, gapOpenCount int, queryStart int, queryEnd int, subjectStart int, subjectEnd int, eVal float, bitScore float)"| mysql -B -uhack -pHackCVR16 Hack
